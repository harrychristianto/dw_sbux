...
with pos as (
  ...
  {% if is_incremental() %}
    where ingestion_ts > (
      select coalesce(max(ingestion_ts), to_timestamp_ntz('1970-01-01'))
      from {{ this }}
    )
  {% endif %}
),
delivery as (
  select
    cast(delivery_order_id as varchar) as transaction_id,
    cast(store_id as varchar)          as store_id,
...
select
  ...
  channel,
  order_type,
  /* Net sales = subtotal - discount + service + tax */
